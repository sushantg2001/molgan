...
class MolecularMetrics(object):
    ...
    # @staticmethod
    # def write_result(mols, smiles, results):
    #     f = open("logs.txt", 'a')
    #     f.write("\n New Result \n")
    #     for i in range(len(mols)):
    #         f.write(
    #             f"{smiles[i]}   {round(results[i],4)}\n")
    #     f.write(f"Accuracy - {MolecularMetrics.odor_score(results)}\n")

    @staticmethod
    def odor(mols,m1_output, m1_input):
        smiles = []
        temp1 = []
        temp2 = []
        count = 0
        ...
        df.to_csv(f'{m1_input}/{name}.csv', index=False)
        os.system(
            f"python m1/transformer-cnn.py  '{m1_input}/{name}.csv' '{m1_output}/{name}.csv' m1")
        results = pd.read_csv(f'{m1_output}/{name}.csv')
        temp = []
    ...
    @staticmethod
    def odor_score(mols, m1_output, m1_input):
        results = MolecularMetrics.odor(mols, m1_output, m1_input)
        one = 0
        total = 0
...
def all_scores(mols, m1_output, m1_input, data, norm=False, reconstruction=False):
    ...
    m1 = {'valid score': MolecularMetrics.valid_total_score(mols) * 100,
        ...
        #   'odor score': MolecularMetrics.odor_score(mols, m1_output, m1_input)
          }
