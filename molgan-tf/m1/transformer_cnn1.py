...
from graph_layers import LayerNormalization, MaskLayerLeft, MaskLayerRight, MaskLayerTriangular, PositionLayer, SelfLayer
version = 4
...
# RESULT_FILE = getConfig("Task","result_file", f"results.csv")
...
TRAIN = "False"
MODEL_FILE = f'model56.tar'
# TRAIN_FILE = getConfig("Task","train_data_file")
APPLY_FILE = f'input.csv'
...
def findBoundaries(DS):
    ...
    for prop in props:
        ...
        if np.count_nonzero(hist) > 2:
            ...
            print(props[prop][1], "regression:",
                  y_min, "to", y_max, "scaling...")
...
def buildNetwork():
    ...
    encoder.set_weights(np.load(f"embeddings.npy", allow_pickle=True))
    ...
    if TRAIN == "True":
        ...
        if len(canon_pairs) > 0:
                ...
                def on_epoch_end(self, epoch, logs={}):
                    if epoch in epochs_to_save:
                        smi2smi.save_weights(
                            f"tr-" + str(epoch) + ".h5", save_format="h5")
            ...
            for i in epochs_to_save:
                f.append(h5py.File(f"tr-" + str(i) + ".h5", "r+"))
            ...
            for i in epochs_to_save[1:]:
                os.remove(f"tr-" + str(i) + ".h5")
            os.rename(f"tr-" +
                      str(epochs_to_save[0]) + ".h5", f"final.h5")

            # extract embeddings
            smi2smi.load_weights(f"final.h5")
            w = smi_encoder.get_weights()
            np.save(f"embeddings.npy", w)
            os.remove(f"final.h5")

        else:
            if CHIRALITY == "True":
                shutil.copy("pretrained/embeddings.npy", f"embeddings.npy")
            else:
        ...
        class MessagerCallback(tf.keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs={}):
                ...
                if EARLY_STOPPING == 0 and epoch >= (NUM_EPOCHS - AVERAGING - 1):
                    self.model.save_weights(f"e-" + str(epoch) + ".h5")
        ...
        if EARLY_STOPPING > 0:
            ...
            mdl.load_weights(f"model/")  # restoring best saved model
            mdl.save_weights(f"model.h5")
            ...
            for i in range(NUM_EPOCHS - AVERAGING - 1, NUM_EPOCHS):
                f.append(h5py.File(f"e-" + str(i) + ".h5", "r+"))
            ...
            for i in range(NUM_EPOCHS - AVERAGING, NUM_EPOCHS):
                os.remove(f"e-" + str(i) + ".h5")
            os.rename(f"e-" + str(NUM_EPOCHS - AVERAGING - 1) +
                      ".h5", f"model.h5")
        ...
        tar.add(f"model.pkl")
        tar.add(f"model.h5")
        tar.add(f"embeddings.npy")
        tar.close()

        if EARLY_STOPPING > 0:
            shutil.rmtree("model/")

        os.remove(f"model.pkl")
        os.remove(f"model.h5")
        os.remove(f"embeddings.npy")

    elif TRAIN == "False":

        tar = tarfile.open(MODEL_FILE)
        tar.extractall(path="/content/drive/MyDrive/molgan/molgan/m1")
        tar.close()

        props = pickle.load(open(f"model.pkl", "rb"))

        mdl, encoder = buildNetwork()
        mdl.load_weights(f"model.h5")
